...
function mapMessageHandler(msg) {
    ...
    case 'Elev':
        if (isMapCreated) {
            ...
            const elevations = source._data.geometry.coordinates.map(v = >map.queryTerrainElevation(v));
    ...
    case 'LandUse':
        ...
        if (isMapCreated) {
            const elevations = msg.data.map(v = >map.queryTerrainElevation(v));
            //console.log(elevations);
            app.ports.mapResponses.send({
                'msg': 'landuse',
                'elevations': elevations
...
function storageMessageHandler(msg) {
    ...
    case 'storage.get':
        ...
        app.ports.storageResponses.send({
            'msg': 'storage.got',
            'key': msg.key,
            'value': val
        });
...
function setClickMode(newMode, points) {
    ...
    clickToDrag = newMode; {
        ...
        if (clickToDrag) {
            ...
            map.off('mouseenter', 'points').off('mouseleave', 'points').off('mousedown', 'points').off('click', 'points')
        }
    };
};

var orangeMarker = new mapboxgl.Marker({
    color: "#FFA500"
});
...
var whiteMarker = new mapboxgl.Marker({
    color: "#FFFFFF",
    scale: 0.8
});
...
function addOptionals(msg) {
    ...
    if (typeof(msg.orange) != 'undefined') {
        orangeMarker.remove().setLngLat([msg.orange.lon, msg.orange.lat]).addTo(map);
    }

    purpleMarker.remove();
    if (typeof(msg.purple) != 'undefined') {
        purpleMarker.setLngLat([msg.purple.lon, msg.purple.lat]).addTo(map);
    };
...
function profileAsChart(canvasContainerDiv, chartInfo) {
    ...
    if (chart != undefined && chart != null) {
        ...
        chart = new Chart(document.getElementById(canvasId), chartInfo);
    };
}
